...
class SSToDSManager:
    def __init__(self):
        ...
        self._xLFOOT_W_norm: Optional[NDArray[np.float32]] = None
        self._xRFOOT_W_norm: Optional[NDArray[np.float32]] = None
    ...
    def build_swing_func(self, p_W: Mapping[str, Vector3], q_W: Mapping[str, Quaternion]) -> None:
        ...
        p_S_baselink_target: NDArray[np.float32] = self._calc_p_S_baselink_target(
            baselink_move_dist_xB, p_W, q_W)
        p_S_stance: NDArray[np.float32] = self._calc_p_S_stance(p_W, q_W)
        p_S_swing_end: NDArray[np.float32] = self._calc_p_S_swing_end(
            p_S_baselink_target, p_S_stance)
        p_W_swing_start: NDArray[np.float32] = self._calc_p_W_swing_start(
            p_W, q_W)
    ...
    def _build_swing_of_s(self, p_W_swing_start: NDArray[np.float32], p_S_swing_end: NDArray[np.float32]) -> NDArray[object]:
        # p_S_swing_start and p_S_swing_end are at foot mid, need to shift to foot front(the target point)
        p_S_swing_start = np.array(
            [p_W_swing_start[0], p_W_swing_start[1], 0.0], dtype=np.float32)
        p_S_swing_mid = (p_S_swing_start + p_S_swing_end) / 2.0
        p_W_swing_mid = np.array(
            [p_S_swing_mid[0], p_S_swing_mid[1], Config.SWING_TRAJECTORY_MID_HEIGHT], dtype=np.float32)
    ...
    def _calc_p_S_baselink_target(self, baselink_move: float, p_W: Mapping[str, Vector3], q_W: Mapping[str, Quaternion]) -> NDArray[np.float32]:
    ...
    def _calc_p_S_stance(self, p_W: Mapping[str, Vector3], q_W: Mapping[str, Quaternion]) -> NDArray[np.float32]:
        if self._stance_side == "left":
            self._xLFOOT_W_norm = self._calc_xFOOT_W_norm(q_W["l_foot"])
            p_W_l_foot = np.array(
                [p_W["l_foot"].x, p_W["l_foot"].y, p_W["l_foot"].z], dtype=np.float32)
            p_W_l_foot_mid = p_W_l_foot - Config.FOOT_LINK_X_SEMI_LENGTH * self._xLFOOT_W_norm
            p_S_stance = np.array(
                [p_W_l_foot_mid[0], p_W_l_foot_mid[1], 0.0], dtype=np.float32)
        elif self._stance_side == "right":
            self._xRFOOT_W_norm = self._calc_xFOOT_W_norm(q_W["r_foot"])
            p_W_r_foot = np.array(
                [p_W["r_foot"].x, p_W["r_foot"].y, p_W["r_foot"].z], dtype=np.float32)
            p_W_r_foot_mid = p_W_r_foot - Config.FOOT_LINK_X_SEMI_LENGTH * self._xRFOOT_W_norm
            p_S_stance = np.array(
                [p_W_r_foot_mid[0], p_W_r_foot_mid[1], 0.0], dtype=np.float32)
        else:
    ...
    def _calc_p_S_swing_end(self, p_S_baselink_target: NDArray[np.float32], p_S_stance: NDArray[np.float32]) -> NDArray[np.float32]:
        return (p_S_baselink_target - (1-LAMBDA_FOR_SWING_END) * p_S_stance) / LAMBDA_FOR_SWING_END

    def _calc_p_W_swing_start(self, p_W: Mapping[str, Vector3], q_W: Mapping[str, Quaternion]) -> NDArray[np.float32]:
        if self._swing_side == "left":
            self._xLFOOT_W_norm = self._calc_xFOOT_W_norm(q_W["l_foot"])
            p_W_l_foot = np.array(
                [p_W["l_foot"].x, p_W["l_foot"].y, p_W["l_foot"].z], dtype=np.float32)
